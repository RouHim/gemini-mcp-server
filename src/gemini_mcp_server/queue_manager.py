...
class AsyncRequestQueue:
        ...
        async with self._semaphore:
            ...
            try:
                ...
                if request.retry_count <= request.max_retries:
                    logger.warning(f"Request {request.id} failed (attempt {request.retry_count}), will retry: {e}")
                    request.status = RequestStatus.PENDING
                    # Re-queue for retry
                    priority_value = self._get_priority_value(request.priority)
                    await self._queue.put((priority_value, time.time(), request))
                else:
                    logger.error(f"Request {request.id} failed permanently after {request.retry_count} attempts: {e}")
    ...
    async def wait_for_completion(self, request_id: str, timeout: Optional[float] = None) -> QueuedRequest:
        """
        ...
        while True:
            ...
            if timeout and (time.time() - start_time) > timeout:
                raise asyncio.TimeoutError(f"Request {request_id} did not complete within {timeout} seconds")
