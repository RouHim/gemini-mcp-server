...
from .retry_handler import get_user_friendly_error_message, create_structured_error_response
...
class HistoryRequest(BaseModel):
    ...
    success_only: bool = Field(default=False, description="Only return successful generations")


class SearchHistoryRequest(BaseModel):
    """Request for searching generation history."""
    search_term: str = Field(..., description="Search term to look for in prompts")
    limit: int = Field(default=10, ge=1, le=50, description="Number of records to return")
...
class ExportHistoryRequest(BaseModel):
    ...
    include_files: bool = Field(default=False, description="Include base64 encoded image data")
...
async def handle_generate_image(arguments: dict[str, Any] | None) -> list[TextContent | ImageContent | EmbeddedResource]:
    ...
    try:
        ...
        request_id = await queue.enqueue(
            _generate_image_with_history,
            request,
            priority=RequestPriority.NORMAL
        )
        
        # Wait for completion
        completed_request = await queue.wait_for_completion(request_id, timeout=300)  # 5 minutes
        ...
        if completed_request.status.value == "completed" and completed_request.result:
            ...
            return [
                TextContent(
                    type="text",
                    text=f"Error generating image: {error_msg}"
                )
            ]
...
async def handle_get_generation_history(arguments: dict[str, Any] | None) -> list[TextContent]:
    ...
    try:
        ...
        return [
            TextContent(
                type="text",
                text=f"Error getting generation history: {str(e)}"
            )
        ]


async def handle_search_generation_history(arguments: dict[str, Any] | None) -> list[TextContent]:
    ...
    try:
        ...
        if not results:
            return [
                TextContent(
                    type="text",
                    text=f"No results found for '{request.search_term}'"
                )
            ]
        ...
        return [
            TextContent(
                type="text",
                text=f"Error searching generation history: {str(e)}"
            )
        ]
...
async def handle_get_generation_statistics() -> list[TextContent]:
    ...
    try:
        ...
        stats_text += f"Storage used: {stats.get('total_storage_size_mb', 0.0):.1f} MB\n"
        stats_text += f"Recent activity (7 days): {stats.get('recent_generations_7_days', 0)}\n\n"
        ...
        return [
            TextContent(
                type="text",
                text=f"Error getting generation statistics: {str(e)}"
            )
        ]


async def handle_export_generation_history(arguments: dict[str, Any] | None) -> list[TextContent]:
    ...
    try:
        ...
        return [
            TextContent(
                type="text",
                text=f"Error exporting generation history: {str(e)}"
            )
        ]


async def handle_cleanup_old_files(arguments: dict[str, Any] | None) -> list[TextContent]:
    ...
    try:
        ...
        if "error" in result:
            return [
                TextContent(
                    type="text",
                    text=f"Cleanup error: {result['error']}"
                )
            ]
