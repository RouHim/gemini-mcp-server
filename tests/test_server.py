...
class TestMCPServer:
    ...
    async def test_search_generation_history_tool(self):
        ...
        with patch("gemini_mcp_server.server.history_manager") as mock_history:
            mock_history.search_generations = AsyncMock(
                return_value=[
            ...
            result = await handle_call_tool(
                "search_generation_history", {"search_term": "sun", "limit": 10}
            )
    ...
    async def test_get_generation_statistics_tool(self):
        ...
        with patch("gemini_mcp_server.server.history_manager") as mock_history:
            mock_history.get_statistics = AsyncMock(
                return_value={
    ...
    async def test_cleanup_old_generations_tool(self):
        ...
        with patch("gemini_mcp_server.server.history_manager") as mock_history:
            mock_history.cleanup_old_records = AsyncMock(
                return_value={
                    "deleted_records": 10,
                    "deleted_files": 8,
                    "freed_space_mb": 50.5,
            ...
            result = await handle_call_tool(
                "cleanup_old_generations", {"days_old": 30, "delete_files": True}
            )
    ...
    async def test_get_generation_details_tool(self):
        ...
        with patch("gemini_mcp_server.server.history_manager") as mock_history:
            mock_history.get_generation_details = AsyncMock(
                return_value={
                    "id": "gen-123",
                    "prompt": "test prompt",
                    "status": "completed",
            ...
            result = await handle_call_tool(
                "get_generation_details", {"generation_id": "gen-123"}
            )
