...
class TestHistoryManager:
    ...
    async def test_record_generation(self, history_manager, sample_params):
        ...
        generation_id = await history_manager.record_generation(
            prompt="test prompt", parameters=sample_params
        )
    ...
    @pytest.mark.asyncio
    async def test_update_generation_result_success(
        self, history_manager, sample_params
    ):
    ...
    async def test_mark_generation_failed(self, history_manager, sample_params):
        ...
        await history_manager.mark_generation_failed(
            generation_id=generation_id, error_message="Test error", retry_count=2
        )
    ...
    @pytest.mark.asyncio
    async def test_search_generations_with_filters(
        self, history_manager, sample_params
    ):
        """Test searching with status and date filters."""
        # Create generations with different statuses
        success_id = await history_manager.record_generation(
            "success prompt", sample_params
        )
        await history_manager.update_generation_result(
            success_id,
            {
        ...
        failed_id = await history_manager.record_generation(
            "failed prompt", sample_params
        )
        ...
        with patch("builtins.open", create=True):
            results = await history_manager.search_generations(
                search_term="prompt", status_filter="completed", limit=10
            )
    ...
    async def test_get_statistics(self, history_manager, sample_params):
        ...
        # Create test data
        for i in range(5):
            gen_id = await history_manager.record_generation(
                f"prompt {i}", sample_params
            )
    ...
    async def test_export_history_json(
        self, history_manager, sample_params, temp_images_dir
    ):
        ...
        with patch("builtins.open", create=True) as mock_export_open:
            ...
            export_path = await history_manager.export_history(
                format="json", include_files=False
            )
    ...
    async def test_export_history_csv(self, history_manager, sample_params):
        ...
        with (
            ...
            export_path = await history_manager.export_history(
                format="csv", include_files=False
            )
    ...
    async def test_cleanup_old_records(
        ...
        with patch("gemini_mcp_server.history_manager.datetime") as mock_datetime:
            ...
            old_id = await history_manager.record_generation(
                "old prompt", sample_params
            )

        # Create recent record normally
        recent_id = await history_manager.record_generation(
            "recent prompt", sample_params
        )
        ...
        await history_manager._execute_query(
            "UPDATE generations SET file_path = ? WHERE id = ?", (old_file_path, old_id)
        )
        await history_manager._execute_query(
    ...
    @pytest.mark.asyncio
    async def test_file_cleanup_on_failure(
        self, history_manager, sample_params, temp_images_dir
    ):
